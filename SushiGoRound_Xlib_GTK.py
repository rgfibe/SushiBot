...
Xlib and gtk modules are Linux only.
...
import gtk.gdk # replaces ImageGrab.
import Xlib.display, Xlib.ext.xtest, Xlib.X # replaces win32api.
import threading # parallel thread sleeps to wait for food delivery.

class Glob:
# This class is just a big dictionary for coordinates and pixel values.

    # GIMP Pixel ruler on full-screen capture to get game window size.
    window_topleft = (33, 271) 
    # window_bottomright = (673, 750)

    # util.get_mousepos_and_pixel() to get all values below, 
...
class Util:
    ...
    def silence(self):
        # Only way to silence Xlib, redirect to '/dev/null'.
    ...
    def screen_grab(self, topleft, xlen, ylen):
        ...
        pb = gtk.gdk.Pixbuf(gtk.gdk.COLORSPACE_RGB, False, 8, xlen, ylen)
        # topleft is relative to Glob.window_topleft, 
        # needs to be adjusted.
        pb = pb.get_from_drawable(window, window.get_colormap(),
                                  topleft[0] + Glob.window_topleft[0],
                                  topleft[1] + Glob.window_topleft[1],
                                  0, 0, xlen, ylen)
        #pb.save(os.getcwd() + '//ScreenGrabs//full_snap__' + 
        #        str(int(time.time())) + '.png', 'png')

        # Note: pixels_array in column-major order, 
        # if more than one pixel have to reverse x and y! 
        return pb.get_pixels_array()

    def get_pix(self, posxy):
    # Returns RGB value list for single pixel at posxy
        return  self.screen_grab(posxy, 1, 1)[0][0]
    ...
    def pix_sum(self):
        pix_grab = self.screen_grab(self.topleft, self.xlen, self.ylen)
        return sum(self.flatten(pix_grab))

    def get_mouse_pos(self):
        pos = [None, None]
        self.silence()
        display = Xlib.display.Display().screen().\
                  root.query_pointer()._data
        self.verbose()
        pos[0] = display["root_x"] - Glob.window_topleft[0]
        pos[1] = display["root_y"] - Glob.window_topleft[1]
        print 'Mouse is at:', 'x =', pos[0], 'y =', pos[1]
        return pos
    ...
    def get_mousepos_and_pixel(self):
...
class Dish(Util):
    ...
    def fold_mat(self):
        ...
        self.clean_crap()
        self.clear_tables()
        time.sleep(0.7) # Mat takes about 1.2 secs to fold.
...
class Seat(Util):
    def __init__(self, name, dish_menu, 
        ...
        self.curr_bloonpix = self.pix_sum()
